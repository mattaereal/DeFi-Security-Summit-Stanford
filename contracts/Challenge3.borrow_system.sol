 ...
 * I peeked a bit the code, just to see what it was.
 */
...
contract BorrowSystemInsecureOracle {
	...
	mapping (address => uint256) public token0Deposited;
	mapping (address => uint256) public token0Borrowed;
	mapping (address => uint256) public token1Deposited;
	mapping (address => uint256) public token1Borrowed;
	...
	function getMaxBorrow(address user) public view returns(uint256) {
		...
		uint256 maxBorow = collateralValue * 100 / 90; // 90% LTV
